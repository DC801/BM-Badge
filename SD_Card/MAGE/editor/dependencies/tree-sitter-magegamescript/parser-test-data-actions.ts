...
export const actionTests: Record<string, ActionTest> = {
	...
	array_map_lambda_chain: {
		input: [`array a = b`, `	.map(($n) { return $n + 1; })`, `	.map(($n) { return $n + 2; });`],
		expected: [
			`array "__TEMP_ARRAY_1" = [];`,
			`"__TEMP_0" = 0;`,
			`"__TEMP_1" = "b".length();`,
			`map_condition_*A*:`,
			`if "__TEMP_0" < "__TEMP_1" then goto label map_body_*A*;`,
			`goto label map_break_*A*;`,
			`map_body_*A*:`,
			`"__TEMP_2" = "b"["__TEMP_0"];`,
			`"__TEMP_3" = "__TEMP_2";`,
			`"__TEMP_3" += 1;`,
			`"__RETURN_" = "__TEMP_3";`,
			`end_of_script_*X*:`,
			`"__TEMP_ARRAY_1".push("__RETURN_");`,
			`"__RETURN_" = 0;`,
			`map_continue_*A*:`,
			`"__TEMP_0" += 1;`,
			`goto label map_condition_*A*;`,
			`map_break_*A*:`,
			`array "__TEMP_ARRAY_2" = [];`,
			`"__TEMP_0" = 0;`,
			`"__TEMP_1" = "__TEMP_ARRAY_1".length();`,
			`map_condition_*B*:`,
			`if "__TEMP_0" < "__TEMP_1" then goto label map_body_*B*;`,
			`goto label map_break_*B*;`,
			`map_body_*B*:`,
			`"__TEMP_2" = "__TEMP_ARRAY_1"["__TEMP_0"];`,
			`"__TEMP_3" = "__TEMP_2";`,
			`"__TEMP_3" += 2;`,
			`"__RETURN_" = "__TEMP_3";`,
			`end_of_script_*Y*:`,
			`"__TEMP_ARRAY_2".push("__RETURN_");`,
			`"__RETURN_" = 0;`,
			`map_continue_*B*:`,
			`"__TEMP_0" += 1;`,
			`goto label map_condition_*B*;`,
			`map_break_*B*:`,
			`array "__TEMP_ARRAY_1" = "__TEMP_ARRAY_2".slice();`,
	...
	array_slices_expressions: {
		...
		expected: [
			...
			`__TEMP_0 = player x;`,
			`__TEMP_0 += 10;`,
			`array __TEMP_ARRAY_1 = b.slice(__TEMP_0);`,
			// `"__TEMP_ARRAY_1".rename("a")`,
			`array a = __TEMP_ARRAY_1.slice();`,
			`delete array __TEMP_ARRAY_1;`,
			`array __TEMP_ARRAY_1 = [];`,
			...
			`__TEMP_1 = player y;`,
			`__TEMP_1 -= 10;`,
			`array __TEMP_ARRAY_1 = d.slice(__TEMP_0, __TEMP_1);`,
	...
	array_slices_strings: {
		...
		expected: [
			`array __TEMP_ARRAY_1 = [];`,
			`array __TEMP_ARRAY_1 = b.slice(zero);`,
			// `"__TEMP_ARRAY_1".rename("a")`,
			`array a = __TEMP_ARRAY_1.slice();`,
			`delete array __TEMP_ARRAY_1;`,

			`array __TEMP_ARRAY_1 = [];`,
			`array __TEMP_ARRAY_1 = d.slice(one, two);`,
			// `"__TEMP_ARRAY_1".rename("c")`,
			`array c = __TEMP_ARRAY_1.slice();`,
			`delete array __TEMP_ARRAY_1;`,
			...
			`array __TEMP_ARRAY_1 = [];`,
			`__TEMP_0 = 4;`,
			`array __TEMP_ARRAY_1 = f.slice(three, __TEMP_0);`,
			...
			`array __TEMP_ARRAY_1 = [];`,
			`__TEMP_0 = 5;`,
			`array __TEMP_ARRAY_1 = h.slice(__TEMP_0, six);`,
			// `"__TEMP_ARRAY_1".rename("g")`,
			`array g = __TEMP_ARRAY_1.slice();`,
			`delete array __TEMP_ARRAY_1;`,
		],
	},
