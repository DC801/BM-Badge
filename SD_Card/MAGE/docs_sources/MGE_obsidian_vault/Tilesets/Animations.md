...
In Tiled, animations are assigned to tiles within the [[Tilesets|spritesheet]]. To an assign an animation to a tile, select a tile — preferably something indicative of what the animation will be, such as a first step for a walking animation — and press the button for the animation editor, which looks like a movie camera:
...
**Best Practice:** to avoid robotic synchronization between entities, try to stagger their animation timings by setting the `current_frame` property for that entity on their Tiled map or via [[SET_ENTITY_CURRENT_FRAME]] inside a [[scripts|script]].
...
Keep in mind that these animations can only be deliberately triggered by scripts, either for an arbitrary number of loops with [[PLAY_ENTITY_ANIMATION]], or indefinitely with [[SET_ENTITY_CURRENT_ANIMATION]].
