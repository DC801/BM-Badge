...
Styling for [[MGS Natlang]] [[Serial Dialogs (MGS)|serial dialog]] [[Serial Dialog Messages (MGS)|messages]].

A unique feature of [[Serial Dialogs (MGS)|serial dialog]] [[Serial Dialog Messages (MGS)|messages]] and [[Serial Dialog Options (MGS)|serial options]] is styling. Styles, implemented with ANSI escape codes, are turned on and off with tags enclosed in `<` and `>`.
