...
This tool lets you easily see the relationship between the hex value of a script/entity_type/dialog/etc. and its name. E.g. if an entity's [[on_tick]] was being changed unexpectedly, but you only knew the hex value that it's being changed to, using Kaitai to find the name of the script in question can help you track down the problem.
