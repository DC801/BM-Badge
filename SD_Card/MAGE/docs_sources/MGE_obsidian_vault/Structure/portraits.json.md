...
The top-level string is the name of the portrait. For most cases, it should be the same as the `entity_type` name for the intended [[character entity]].

`tileset` is the file path for the [[tilesets|JSON file]] [[portraits.json|associated]] with the portrait image. The encoder assumes these JSON files will be inside `entities/`.
